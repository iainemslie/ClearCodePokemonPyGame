...
class Entity(pygame.sprite.Sprite):
    ...
    def animate(self, dt):
        ...
        self.image = self.frames[self.get_state()][int(
...
class Character(Entity):
    def __init__(self, pos, frames, groups, facing_direction, character_data, player, create_dialog, collision_sprites, radius, nurse, notice_sound):
        ...
        self.nurse = nurse
        self.monsters = {i: Monster(name, lvl) for i,
                         (name, lvl) in character_data['monsters'].items()} if 'monsters' in character_data else None
        ...
        self.timers = {
            'look_around_timer': Timer(
                int(random() * 5000), repeat=True, autostart=True, func=self.look_around),
    ...
    def has_los(self):
        if vector(self.rect.center).distance_to(self.player.rect.center) < self.radius:
            collisions = [bool(rect.clipline(
                self.rect.center, self.player.rect.center)) for rect in self.collision_rects]
            return not any(collisions)

    def start_move(self):
        relation = (vector(self.player.rect.center) -
                    vector(self.rect.center)).normalize()
